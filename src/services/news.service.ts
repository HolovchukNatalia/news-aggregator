...
export const fetchNews = async (
  ...
  console.log(`🔍 REQUEST → Page ${page} | Endpoint: ${endpoint} | Query: "${searchQuery || 'none'}" | Source: ${source || 'none'}`)
  ...
  if (data.articles) {
    data.articles = data.articles.filter((article: any) => {
      ...
      if (!article.title) {
        console.log(`🚫 BLOCKED: Article with no title from ${article.source.name}`)
        return false
      }
  ...
  console.log(`✅ RESPONSE → Found ${data.totalResults} results, showing ${filteredCount} articles${blockedCount > 0 ? ` (blocked ${blockedCount} Russian sources)` : ''}`)
  if (data.articles?.[0]) {
